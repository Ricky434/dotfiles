#!/bin/sh
# POLYWINS
# github.com/alnj/polywins

# SETTINGS {{{ ---

active_text_color="#b3afc2"
active_bg=
active_underline="#c3e88d"

inactive_text_color="#b3afc2"
inactive_bg=
inactive_underline=
...
generate_window_list() {
	...
	while IFS="[ .\.]" read -r wid ws cname cls host title; do
		...
		#printf "%s" "%{A2:$on_click close $wid:}"   (bisogna aggiungere un %{A} all'ultimo printf)
		#printf "%s" "%{A3:$on_click slop_resize $wid:}"    (uguale a sopra)
